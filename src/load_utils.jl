...
function _load_block!(block::MBConv, params, base)
    ...
    rebuild_conv!(block.depthwise[1].weight, params[base * "._depthwise_conv.weight"])
    ...
    if block.excitation ≢ nothing
        rebuild_conv!(block.excitation[2].weight, params[base * "._se_reduce.weight"])
        copyto!(block.excitation[2].bias, params[base * "._se_reduce.bias"])
        rebuild_conv!(block.excitation[3].weight, params[base * "._se_expand.weight"])
        copyto!(block.excitation[3].bias, params[base * "._se_expand.bias"])
    end

    rebuild_conv!(block.projection[1].weight, params[base * "._project_conv.weight"])
...
@inline function _load_pth!(model::EffNet, params, include_head)
    _load_stem!(model, params)
    _load_blocks!(model, params)
    include_head && _load_head!(model, params)
end

function from_pretrained(model_name; cache_dir = nothing, include_head = true)
